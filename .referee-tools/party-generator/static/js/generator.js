...
const generatorScript = document.currentScript;
const generatorRoot = generatorScript
    ? new URL("../../", generatorScript.src)
...
async function getGameData() {
    ...
    const promises = files.map(file => fetch(new URL(`data/${file}.json`, generatorRoot)).then(response => response.json()));
